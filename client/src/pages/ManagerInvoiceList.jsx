    ...
    <div className="p-6 space-y-6 bg-slate-950 text-slate-100 min-h-[calc(100vh-64px)]">
      ...
      <div>
        ...
        <p className="text-sm text-slate-400">
          Verify vendor invoices against generated Purchase Orders and authorize payments.
        </p>
      </div>
      ...
      {invoices.length > 0 ? (
        <div className="space-y-4">
          {invoices.map((invoice) => (
            <div
              key={invoice.id}
              className={`bg-slate-900 border rounded-xl p-5 hover:border-slate-700 transition ${invoice.status === 'Paid'
            ...
            >
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-800/80 pb-3 mb-4">
                ...
                <span
                  className={`text-[9px] uppercase font-extrabold px-2.5 py-0.5 rounded-full ${invoice.status === 'Paid'
              ...
              {/* Actions */}
              <div className="border-t border-slate-800/80 mt-4 pt-3.5 flex justify-end space-x-2">
                <button
                ...
                {invoice.status === 'Pending' && (
                  <button
                    ...
                    className="inline-flex items-center space-x-1 bg-emerald-600 hover:bg-emerald-700 text-white px-3.5 py-1.5 rounded-lg text-xs font-semibold shadow-md active:translate-y-px transition"
                  >
